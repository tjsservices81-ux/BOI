...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByCustomerNumber(customerNumber: string): Promise<User | undefined> {
    return this.getUser(customerNumber);
  }
  ...
  async deleteChatResponse(id: number): Promise<void> {
    await db.delete(chatResponses).where(eq(chatResponses.id, id));
  }

  async getAllDeviceSessions(): Promise<any[]> {
    // This will be handled by deviceSessions.ts
    return [];
  }

  async blockDeviceSession(sessionId: string): Promise<void> {
    // Handled by panic mode system
  }
  ...
  async disableUser(userId: number): Promise<void> {
    await db.update(users).set({ isDisabled: true }).where(eq(users.id, userId));
  }

  async enableUser(userId: number): Promise<void> {
    await db.update(users).set({ isDisabled: false }).where(eq(users.id, userId));
  }
  ...
  async initializeSampleData(): Promise<void> {
    ...
    const sampleUsers = [
      ...
      },
      {
        ...
        pin: "9012",
        name: "Emma Kelly",
        email: "emma.kelly@example.com", 
        phone: "+353 86 555 7890",
        address: "8 Shop Street, Galway, Ireland",
...
// Memory storage implementation (keeping for migration)
