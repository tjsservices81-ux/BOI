...
function exportData(){
const csv=['Alias,Customer Number,App Replacement'];
allCust.filter(c=>!c.deletedAt).forEach(c=>{
csv.push(\`"\${escapeHtml(c.adminAlias||'')}",\${c.customerNumber},\${c.appReplacement||0}\`);
});
