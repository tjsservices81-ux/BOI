...
import { loginSchema, transferSchema, ukTransferSchema, ukPayments } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // UK Transfer endpoint with sort code validation
  app.post("/api/transfer/uk", requireAuth, async (req, res) => {
    try {
      ...
      // Validate sort code
      const receivingBank = sortCodeToBank[transferData.sortCode];
      if (!receivingBank) {
        return res.status(400).json({ 
          success: false, 
          error: "Unsupported bank – sort code not recognised" 
        });
      }

      // Generate random 6-character payment ID (uppercase letters and numbers)
      ...
      const accountMasked = `****${transferData.accountNumber.slice(-4)}`;
      ...
      const ukPayment = {
        ...
        sortCode: transferData.sortCode,
        receivingBank,
        accountMasked,
        dateTime,
      ...
      if (transferData.recipientPhone) {
        try {
          ...
          const smsMessage = `Bank of Ireland: A payment of ${transferData.amount} has been sent to your account.
From: ${user.name}
Ref: ${transferData.reference}
Sort code: ${transferData.sortCode} (${receivingBank})
Acc: ${accountMasked}
Sent: ${dateTime}
...
View details: ${confirmationLink}`;
          ...
          await twilio.messages.create({
            ...
            to: transferData.recipientPhone
          });

          console.log(`SMS sent to ${transferData.recipientPhone} for payment ${paymentID}`);
        } catch (smsError) {
          console.error('SMS sending failed:', smsError);
          // Continue with transfer even if SMS fails
        }
      }
      ...
      res.json({
        ...
        paymentID,
        confirmationLink,
        message: "UK transfer completed successfully"
      });

    } catch (error) {
      console.error('UK transfer failed:', error);
      if (error instanceof z.ZodError) {
        return res.status(400).json({ 
          success: false, 
  ...
  // Payment confirmation view endpoint
  app.get("/view/:paymentID", async (req, res) => {
    try {
      ...
      if (!payment) {
        return res.status(404).send(`
          ...
          <body>
            ...
            <p>The payment reference ${paymentID} could not be found.</p>
          </body>
          </html>
        `);
      }

      // Bank of Ireland styled confirmation page
      res.send(`
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>Bank of Ireland</h1>
              <p>Payment Confirmation</p>
            </div>
            
            <div class="content">
              <div class="status">Status: ${payment.status}</div>
              ...
              <div class="detail-row">
                ...
                <span class="value">${payment.senderName}</span>
              </div>
              ...
              <div class="detail-row">
                ...
                <span class="value">${payment.sortCode}</span>
              </div>
              
              <div class="detail-row">
                <span class="label">Receiving Bank:</span>
                <span class="value">${payment.receivingBank}</span>
              </div>
              
              <div class="detail-row">
                <span class="label">Account Number:</span>
                <span class="value">${payment.accountMasked}</span>
              </div>
              
              <div class="detail-row">
                <span class="label">Date & Time:</span>
                <span class="value">${payment.dateTime}</span>
              </div>
              ...
              <div class="note">
                <strong>Note:</strong> Funds may take 2–24 hours to clear depending on your bank.
              </div>
            </div>
            
            <div class="footer">
              © Bank of Ireland. All rights reserved.
            </div>
          </div>
        </body>
        </html>
