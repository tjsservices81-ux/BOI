...
async function create2FreshCodes() {
  console.log('🔑 CREATING 2 FRESH BOI ACCESS CODES\n');
  ...
  console.log('\n📋 NEW FRESH ACCESS CODES:');
  codes.forEach((code, index) => {
    console.log(`  ${index + 1}. ${code} (All devices: 2 uses each)`);
  });
  
  console.log('\n🎉 FRESH ACCESS CODES CREATED SUCCESSFULLY');
  console.log('✅ All device types have 2 uses per code');
  console.log('✅ iOS: 2 uses | Android: 2 uses | Others: 2 uses');
}

create2FreshCodes().catch(console.error);