        ...
        <div className="bg-white rounded-t-3xl h-full">
          <div className="pt-6" style={{ overscrollBehavior: 'contain' }}>
            {(accounts && Array.isArray(accounts)) && accounts.map((account, index) => (
              <button 
                key={account.id}
                className={`w-full flex items-center justify-between border-b border-gray-100 hover:bg-gray-50 touch-manipulation transform-gpu transition-all duration-150 ease-out active:scale-98 haptic-feedback relative stagger-item card-interactive android-no-highlight ${isNavigating ? 'pointer-events-none' : ''}`}
