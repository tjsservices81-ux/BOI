              ...
              <div className="mb-6 bg-gray-50 rounded-2xl p-5 border border-gray-200 shadow-sm">
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {/* International Transfer (includes Mexico/CLABE) */}
                    <button
                      onClick={() => {
                        const currentlyOn = transferSettings.showInternationalTransfer !== false;
                        const newSettings = { ...transferSettings, showInternationalTransfer: !currentlyOn };
                        setTransferSettings(newSettings);
                        UserDataManager.setUserData('transferSettings', newSettings);
                        UserDataManager.clearCache('transferSettings');
                        window.dispatchEvent(new CustomEvent('transferSettingsUpdate'));
                        showDeveloperMessage(`International Transfer ${newSettings.showInternationalTransfer ? 'enabled' : 'disabled'} successfully`);
                      }}
                      data-testid="toggle-international-transfer"
                      className="w-full flex items-center justify-between p-3 bg-white/70 backdrop-blur-sm border-2 border-teal-200 rounded-xl active:scale-95 transition-all shadow-sm hover:shadow-md"
                    >
                      <div className="flex-1 text-left">
                        <p className="font-semibold text-teal-900 text-sm" style={{ fontFamily: 'OpenSans, sans-serif' }}>
                        ...
                        <p className="text-xs text-teal-600" style={{ fontFamily: 'OpenSans, sans-serif' }}>
                          USA, Canada, Australia, Mexico (CLABE)
                        </p>
                      </div>
                      <div className={`w-11 h-6 rounded-full transition-colors ${transferSettings.showInternationalTransfer !== false ? 'bg-green-500' : 'bg-gray-300'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform mt-1 ${transferSettings.showInternationalTransfer !== false ? 'translate-x-6' : 'translate-x-1'}`} />
                      </div>
                    </button>
