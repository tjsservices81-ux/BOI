...
- **Settings & Preferences**: User-controlled notification and email settings that properly gate email/notification sending; toggles for enabling/disabling transfer confirmations, bank statement emails, and push notifications; "Show Transfer Confirmation" toggle in settings to hide/show the "Open Transfer Confirmation" button on transaction details modals (excludes custom transactions).
