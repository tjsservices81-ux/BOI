...
- **Authentication**: Session-based with 1-year persistent cookies; PostgreSQL session store with connect-pg-simple (resave: true, saveUninitialized: false) for reliable session persistence; OfflineAuthGuard system with 5-location backup (localStorage x3 + sessionStorage x2) ensures users stay logged in through all network failures, WiFi loss, server restarts, and browser crashes until admin deletion; all User objects include customerNumber for proper data isolation; lenient device authorization (allows same device type: iOS→iOS, Android→Android) prevents lockouts from browser updates.
...
- **Security**: Device-specific one-time access codes, PIN-based authentication, session validation; automatic logout ONLY when customer is deleted from admin panel (via checkCustomerExists() in heartbeat); per-user data isolation (localStorage stores only authenticated user's data, not all customers); secure `/api/users/me` endpoint requires authentication before returning user data; customerNumber required in all user contexts for isolation; fail-safe error handling prevents false logouts on database errors, network failures, and malformed responses; SINGLE logout path: heartbeat customer deletion with explicit logout:true/forceDisconnect:true flags; all other errors (403, network issues, JSON parse failures) keep user logged in; Twilio secrets loaded from environment variables only (no hard-coded secrets).
